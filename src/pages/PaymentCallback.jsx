...
import { verifyPayTRCallback } from '../lib/paytrService';
import { sendPaymentSuccessNotification } from '../lib/notificationService';
  ...
  useEffect(() => {
    const handlePaymentCallback = async () => {
      try {
        ...
        const callbackData = {
          merchant_oid: searchParams.get('merchant_oid'),
          status: searchParams.get('status'),
          total_amount: searchParams.get('total_amount'),
          hash: searchParams.get('hash'),
          payment_id: searchParams.get('payment_id'),
          installment_count: searchParams.get('installment_count'),
          payment_type: searchParams.get('payment_type')
        };

        // PayTR callback'ini doğrula
        const verificationResult = await verifyPayTRCallback(callbackData);
        ...
        if (verificationResult.success) {
          if (verificationResult.status === 'success') {
            ...
            setStatus('success');
            setOrderData({
              orderNumber: verificationResult.order_id,
            ...
            await sendPaymentSuccessNotification({
              order_number: verificationResult.order_id,
              total_amount: callbackData.total_amount,
              customer_phone: '' // Buraya kullanıcı telefonu gelecek
            }, isEnglish);
