...
const Appointment = () => {
  ...
  // Fetch available dates (dates that have at least one available slot)
  const fetchAvailableDates = async () => {
    try {
      ...
      appointments?.forEach(appointment => {
        const key = `${appointment.appointment_date}-${appointment.appointment_time}`;
      ...
      slots?.forEach(slot => {
        if (!dateAvailability[slot.date]) {
          dateAvailability[slot.date] = { hasAvailability: false };
        }
        
        // Check if this slot has availability
        const slotKey = `${slot.date}-${slot.start_time}`;
        const currentBookings = appointmentCounts[slotKey] || 0;
        
        if (currentBookings < slot.max_appointments) {
          dateAvailability[slot.date].hasAvailability = true;
        }
      });

      // Get unique dates that have availability
      const uniqueDates = Object.entries(dateAvailability)
  ...
  const fetchAvailableSlots = async (date) => {
    ...
    try {
      ...
      const availableSlots = slots?.filter(slot => {
        const currentBookings = appointmentCounts[slot.start_time] || 0;
                  ...
                  <div className="row">
                    {appointmentTypes.map(type => (
                      <div key={type.id} className="col-md-6 mb-3">
                        <div 
                          className={`card h-100 appointment-type-card ${formData.appointmentTypeId === type.id ? 'selected' : ''}`}
              ...
              <div className="card">
                ...
                <div className="card-body">
                  <form onSubmit={handleSubmit}>
                    ...
                    {/* Contact Form - Sadece telefon numarası */}
                    <div className="mb-3">
