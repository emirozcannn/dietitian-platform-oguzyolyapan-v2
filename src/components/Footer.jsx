...
const Footer = () => {
  ...
  // Newsletter aboneliği fonksiyonu
  const handleNewsletterSubmit = async (e) => {
    e.preventDefault();
    
    if (!newsletterEmail) {
      setNewsletterStatus('error');
      setNewsletterMessage(isEnglish ? 'Please enter your email address' : 'Lütfen e-posta adresinizi girin');
      return;
    }
    ...
    try {
      // Newsletter aboneliğini veritabanına kaydet
      ...
      if (existingSubscriber) {
        if (existingSubscriber.is_active) {
          setNewsletterStatus('error');
          setNewsletterMessage(isEnglish ? 'You are already subscribed to our newsletter' : 'Zaten bültenimize abone olmuşsunuz');
          return;
        } else {
          // Pasif aboneyi aktif et
      ...
      console.log('Newsletter subscription successful for:', newsletterEmail);

      // Hoş geldin emaili gönder
      const emailResult = await sendNewsletterWelcome(newsletterEmail, isEnglish);
      
      if (emailResult.success) {
        setNewsletterStatus('success');
        setNewsletterMessage(isEnglish ? 'Thank you! Welcome email sent successfully.' : 'Teşekkürler! Hoş geldin emaili gönderildi.');
        setNewsletterEmail('');
      } else {
        setNewsletterStatus('success');
        setNewsletterMessage(isEnglish ? 'Subscribed successfully! Welcome email will be sent shortly.' : 'Başarıyla abone oldunuz! Hoş geldin emaili kısa sürede gönderilecek.');
        setNewsletterEmail('');
      }
      ...
      if (error.message?.includes('relation "newsletter_subscribers" does not exist')) {
        setNewsletterMessage(isEnglish ? 'Database setup required. Please contact support.' : 'Veritabanı kurulumu gerekli. Lütfen destek ile iletişime geçin.');
      } else if (error.message?.includes('row-level security policy') || error.code === '42501') {
        setNewsletterMessage(isEnglish ? 'Database permission issue. Please try again or contact support.' : 'Veritabanı izin sorunu. Lütfen tekrar deneyin veya destek ile iletişime geçin.');
      } else if (error.message?.includes('permission denied')) {
        setNewsletterMessage(isEnglish ? 'Permission error. Please try again.' : 'İzin hatası. Lütfen tekrar deneyin.');
      } else {
        setNewsletterMessage(isEnglish ? 'An error occurred. Please try again.' : 'Bir hata oluştu. Lütfen tekrar deneyin.');
      }
    }
  };
